...
function findAppCheckPaths() {
  ...
  const candidates = [
    path.join(cwd, 'node_modules/@react-native-firebase/app-check/ios/RNFBAppCheck/RNFBAppCheckModule.h'),
    path.join(cwd, 'node_modules/@react-native-firebase/app-check/ios/RNFBAppCheck/RNFBAppCheckModule.m'),
  ];
  try {
    ...
    if (fs.existsSync(dir)) {
      ...
      for (const e of entries) {
        if (e.isDirectory() && e.name.includes('@react-native-firebase+app-check')) {
          const base = path.join(dir, e.name, 'node_modules/@react-native-firebase/app-check/ios/RNFBAppCheck');
          candidates.push(path.join(base, 'RNFBAppCheckModule.h'), path.join(base, 'RNFBAppCheckModule.m'));
        }
      }
    }
  } catch (_) {}
...
function patchRNFBAppCheckHeader() {
  ...
  try {
    ...
    if (!content.includes(rnfbAppImport)) {
      ...
      console.log('[RNFB AppCheck Specific] Added RNFBAppModule import before RCTBridgeModule');
    }
...
`;
    ...
    const methodFixes = [
      ...
      },
      {
        ...
        pattern: /RCT_EXPORT_METHOD\(\s*setTokenAutoRefreshEnabled\s*:\s*\(FIRApp\s*\*\)\s*firebaseApp\s*\n\s*:\s*\(BOOL\)\s*isTokenAutoRefreshEnabled\s*\)/g,
    ...
    console.error('[RNFB AppCheck Specific] Error with implementation:', error.message);
  }
}

function runSpecificFixes() {
  console.log('[RNFB AppCheck Specific] Applying specific fixes for build errors...');
