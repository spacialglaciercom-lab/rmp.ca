...
const CLEANUP_PATTERNS = {
  ...
  monitorScripts: ["monitor-current-build.js", "monitor-ios-build.js"],

  // Cache directories
  cacheDirectories: [".expo", "node_modules/.cache", "dist"],
};
...
function analyzeCleanup() {
  ...
  analysis.migrationScripts = findFilesByPattern(
    CLEANUP_PATTERNS.migrationScripts,
  );
  ...
  analysis.cacheDirectories = findDirectories(
    CLEANUP_PATTERNS.cacheDirectories,
  );
...
function displayAnalysis(analysis) {
  ...
  const categories = [
    {
      name: "Backup & Temporary Files",
      files: analysis.backupFiles,
      color: "🗂️",
    },
    {
      name: "Migration Scripts",
      files: analysis.migrationScripts,
      color: "📦",
    },
    ...
    { name: "Monitor Scripts", files: analysis.monitorScripts, color: "📊" },
    {
      ...
      files: analysis.cacheDirectories,
      color: "🗑️",
    },
  ];
  ...
  categories.forEach((category) => {
    if (category.files.length > 0) {
      console.log(
        `${category.color} ${category.name}: ${category.files.length} items`,
      );
  ...
  console.log(
    `💾 Total space that can be freed: ${formatFileSize(analysis.totalSize)}`,
  );
...
function performCleanup(analysis, dryRun = true) {
  ...
  console.log(
    `📁 ${dryRun ? "Would clean" : "Cleaned"}: ${cleanedCount} items`,
  );
  console.log(
    `💾 ${dryRun ? "Would free" : "Freed"}: ${formatFileSize(freedSpace)}`,
  );
}
...
function createCleanupReport(analysis) {
  const report = {
    ...
    summary: {
      totalItems:
        analysis.backupFiles.length +
        analysis.migrationScripts.length +
        analysis.buildArtifacts.length +
        analysis.monitorScripts.length +
