...
class CostModelTrainer {
  ...
  async train(): Promise<TrainedModel | null> {
    ...
    try {
      ...
      console.log(
        `✅ Preprocessed ${processedData.length} valid training samples`,
      );
      ...
      .map((entry) => {
        ...
        return {
          features: {
            ...
            roadConditions: this.encodeRoadConditions(
              routeChars.roadConditions,
            ),
          },
          targets: {
            ...
            actualCostPerKm:
              entry.actualCosts.totalCost / routeStats.totalDistance,
            actualTimePerKm:
              entry.actualCosts.actualTime / routeStats.totalDistance,
            actualFuelPerKm:
              entry.actualCosts.actualFuelConsumption /
  ...
  private isValidTrainingData(data: TrainingData): boolean {
    ...
    if (features.routeDistance <= 0 || features.routeDistance > 100)
      return false;
    if (features.estimatedTime <= 0 || features.estimatedTime > 600)
      return false;
  ...
  private splitData(processedData: TrainingData[]): void {
    ...
    const splitIndex = Math.floor(
      shuffled.length * (1 - this.options.validationSplit),
    );
  ...
  private async trainAIModel(): Promise<TrainedModel | null> {
    ...
    if (process.platform !== "darwin") {
      console.warn(
        "⚠️  AI training only available on iOS/macOS, falling back to linear model",
      );
      return await this.trainLinearModel();
    }
  ...
  private calculateFeatureCorrelation(featureName: string): number {
    const featureValues = this.trainingData.map(
      (d) => d.features[featureName as keyof typeof d.features],
    );
  ...
  private predictEnsemble(
    ...
    return {
      timeCorrection:
        linearPrediction.timeCorrection * ensembleWeight +
        ruleBasedAdjustment.time * ruleWeight,
      costCorrection:
        linearPrediction.costCorrection * ensembleWeight +
        ruleBasedAdjustment.cost * ruleWeight,
      fuelCorrection:
        linearPrediction.fuelCorrection * ensembleWeight +
        ruleBasedAdjustment.fuel * ruleWeight,
    };
  }

  private getRuleBasedAdjustment(features: TrainingData["features"]): {
    time: number;
    cost: number;
    fuel: number;
  } {
    ...
    if (features.driverExperience === 0) {
      // Novice
      timeAdj += 0.2;
      costAdj += 0.15;
      fuelAdj += 0.1;
  ...
  private calculatePerformanceMetrics(
    predictions: Array<{ predicted: any; actual: any }>,
  ): ModelPerformance {
    const timeErrors = predictions.map((p) =>
      Math.abs(p.predicted.timeCorrection - p.actual.timeCorrection),
    );
    const costErrors = predictions.map((p) =>
      Math.abs(p.predicted.costCorrection - p.actual.costCorrection),
    );
    const fuelErrors = predictions.map((p) =>
      Math.abs(p.predicted.fuelCorrection - p.actual.fuelCorrection),
    );

    const avgTimeAccuracy =
      1 - timeErrors.reduce((a, b) => a + b, 0) / timeErrors.length;
    const avgCostAccuracy =
      1 - costErrors.reduce((a, b) => a + b, 0) / costErrors.length;
    const avgFuelAccuracy =
      1 - fuelErrors.reduce((a, b) => a + b, 0) / fuelErrors.length;

    const timeRmse = Math.sqrt(
      timeErrors.reduce((a, b) => a + b * b, 0) / timeErrors.length,
    );
    const costRmse = Math.sqrt(
      costErrors.reduce((a, b) => a + b * b, 0) / costErrors.length,
    );
    const fuelRmse = Math.sqrt(
      fuelErrors.reduce((a, b) => a + b * b, 0) / fuelErrors.length,
    );
    ...
    return {
      ...
      validationSamples: this.validationData.length,
      averageAccuracy:
        (avgTimeAccuracy + avgCostAccuracy + avgFuelAccuracy) / 3,
...
Provide a model that can predict cost corrections based on route characteristics and operational context.`;
  ...
  private analyzeData(data: TrainingData[]): void {
    ...
    features.forEach((feature: string) => {
      const values = data.map(
        (d) => d.features[feature as keyof typeof d.features],
      );
      const avg = values.reduce((a, b) => a + b, 0) / values.length;
      const min = Math.min(...values);
      const max = Math.max(...values);
      console.log(
        `${feature}: avg=${avg.toFixed(2)}, min=${min.toFixed(2)}, max=${max.toFixed(2)}`,
      );
    });
    ...
    const mediumQuality = qualityScores.filter(
      (q) => q >= 0.6 && q < 0.8,
    ).length;
    ...
    console.log(`\nQuality Distribution:`);
    console.log(
      `High quality (≥0.8): ${highQuality} (${((highQuality / data.length) * 100).toFixed(1)}%)`,
    );
    console.log(
      `Medium quality (0.6-0.8): ${mediumQuality} (${((mediumQuality / data.length) * 100).toFixed(1)}%)`,
    );
    console.log(
      `Low quality (<0.6): ${lowQuality} (${((lowQuality / data.length) * 100).toFixed(1)}%)`,
    );
  }
  ...
  private printTrainingSummary(model: TrainedModel): void {
    ...
    console.log("\n📊 Performance Metrics:");
    console.log(
      `Average Accuracy: ${(model.performance.averageAccuracy * 100).toFixed(1)}%`,
    );
    console.log(
      `Time Prediction Accuracy: ${(model.performance.timePredictionAccuracy * 100).toFixed(1)}%`,
    );
    console.log(
      `Cost Prediction Accuracy: ${(model.performance.costPredictionAccuracy * 100).toFixed(1)}%`,
    );
    console.log(
      `Fuel Prediction Accuracy: ${(model.performance.fuelPredictionAccuracy * 100).toFixed(1)}%`,
    );
...
async function main() {
  ...
  if (
    ...
    trainingOptions.validationSplit > 0.5
  ) {
