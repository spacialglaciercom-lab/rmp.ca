...
// Insert function before orOptVRP
c = c.replace('function orOptVRP(', twoOptFn + 'function orOptVRP(');

// Insert dispatch before the or_opt dispatch block
c = c.replace('  if (config?.algorithm === "or_opt")', twoOptDispatch + '  if (config?.algorithm === "or_opt")');

// Add to ALGORITHM_OPTIONS after or_opt entry
c = c.replace(
  '  { value: "or_opt", label: "Or-Opt (local search)" },',
  '  { value: "or_opt", label: "Or-Opt (local search)" },' + nl + '  { value: "two_opt", label: "2-Opt (route untangling)" },'
);
