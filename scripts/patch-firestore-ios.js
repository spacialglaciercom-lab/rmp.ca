...
function findFirestoreRoot() {
  try {
    const resolved =
      require.resolve("@react-native-firebase/firestore/package.json");
...
if (!fs.existsSync(headerPath)) {
  console.warn(
    "patch-firestore-ios: RNFBFirestoreCommon.h not found, skipping",
  );
  process.exit(0);
}
...
typedef void (^RCTPromiseRejectBlock)(NSString *code, NSString *message, NSError *error);
  ...
  if (content.includes(importLine)) {
    ...
    console.log(
      "patch-firestore-ios: applied RCTPromiseRejectBlock fix to RNFBFirestoreCommon.h",
    );
  }
}
...
if (fs.existsSync(transactionModulePath)) {
  ...
  if (
    !tmContent.includes(firebaseImport) ||
    !tmContent.includes(commonImport)
  ) {
    ...
    console.log(
      "patch-firestore-ios: applied Firebase/Common imports to RNFBFirestoreTransactionModule.m",
    );
  }
}
...
if (fs.existsSync(firestoreModuleHeaderPath)) {
  ...
  if (!hContent.includes(rnfbAppModuleImport)) {
    ...
    console.log(
      "patch-firestore-ios: applied RNFBAppModule import to RNFBFirestoreModule.h",
    );
  }
}
...
if (fs.existsSync(firestoreModulePath)) {
  ...
  const commonImport = '#import "RNFBFirestoreCommon.h"';
  if (
    !fmContent.includes(firebaseImport) ||
    !fmContent.includes(commonImport)
  ) {
    ...
    console.log(
      "patch-firestore-ios: applied Firebase/Common imports to RNFBFirestoreModule.m",
    );
  }
}
...
const firestoreIosDir = path.join(
  findFirestoreRoot() || "",
  "ios",
  "RNFBFirestore",
);
...
for (const name of moduleFiles) {
  ...
  if (
    !fileContent.includes(firebaseImport) ||
    !fileContent.includes(commonImport)
  ) {
    if (fileContent.includes(firstImportAnchor)) {
      ...
      console.log(
        "patch-firestore-ios: applied Firebase/Common imports to " + name,
      );
    }
  }
}
...
function applyBlockFixes(filePath, fileName) {
  ...
  if (c !== orig) {
    ...
    console.log(
      "patch-firestore-ios: applied block pointer fixes to " + fileName,
    );
  }
}
applyBlockFixes(
  path.join(firestoreIosDir, "RNFBFirestoreDocumentModule.m"),
  "RNFBFirestoreDocumentModule.m",
);
applyBlockFixes(
  path.join(firestoreIosDir, "RNFBFirestoreCollectionModule.m"),
  "RNFBFirestoreCollectionModule.m",
);
