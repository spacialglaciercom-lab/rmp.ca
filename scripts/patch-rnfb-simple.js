...
function patchRNFBAppModule() {
  try {
    const modulePath = 'node_modules/@react-native-firebase/app/ios/RNFBApp/RNFBAppModule.h';
    ...
    for (const headerImport of imports) {
      if (!content.includes(headerImport)) {
        ...
        if (licenseEnd) {
          content = content.replace(licenseEnd[0], licenseEnd[0] + headerImport + '\n');
        }
      }
    }
...
function patchRNFBAppCheckModule() {
  try {
    const modulePath = 'node_modules/@react-native-firebase/app-check/ios/RNFBAppCheck/RNFBAppCheckModule.m';
    const fullPath = path.join(process.cwd(), modulePath);
...
`;
    ...
    console.error('[RNFB Simple Patch] Error with RNFBAppCheck:', error.message);
  }
}

function patchRNFBFirestore() {
  try {
    const modulePath = 'node_modules/@react-native-firebase/firestore/ios/RNFBFirestore/RNFBFirestoreCommon.m';
    const fullPath = path.join(process.cwd(), modulePath);
...
`;
    ...
    console.error('[RNFB Simple Patch] Error with RNFBFirestore:', error.message);
  }
}
