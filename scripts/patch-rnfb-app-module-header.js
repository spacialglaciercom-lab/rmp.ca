...
function patchRNFBAppModuleHeader() {
  try {
    ...
    if (!moduleHeaderPath) {
      console.log(
        "[RNFBApp Module Header Patch] Module header not found, skipping patch",
      );
      return;
    }

    console.log(
      `[RNFBApp Module Header Patch] Found module header at: ${moduleHeaderPath}`,
    );
...
`;
    ...
    if (importSectionEnd) {
      ...
      if (insertIndex > 0) {
        ...
        console.log(
          "[RNFBApp Module Header Patch] Successfully added RCTBridgeModule imports and forward declarations",
        );
      }
    } else {
      ...
      if (commentEnd) {
        ...
        fs.writeFileSync(moduleHeaderPath, content, "utf8");
        console.log(
          "[RNFBApp Module Header Patch] Added imports after license header",
        );
      }
    }
    ...
    if (updatedContent.includes("#import <React/RCTBridgeModule.h>")) {
      console.log(
        "[RNFBApp Module Header Patch] ✅ Successfully patched header file",
      );
    } else {
      console.log(
        "[RNFBApp Module Header Patch] ❌ Header patch may not have worked correctly",
      );
    }
  } catch (error) {
    console.error(
      "[RNFBApp Module Header Patch] Error patching module:",
      error.message,
    );
