...
import { costModelValidator, costModelPerformanceMonitor } from "@/lib/cost-model-validation";
import { CostModelTrainer, type TrainedModel } from "@/scripts/train-cost-model";
...
class NightlyCostModelUpdate {
  ...
  async execute(): Promise<UpdateResult> {
    ...
    try {
      ...
      console.log(`New data collected: ${JSON.stringify(newDataInfo, null, 2)}`);

      // Step 4: Determine if update is needed
      console.log("\n🤔 Determining if model update is needed...");
      const updateDecision = this.shouldUpdateModel(currentMetrics, newDataInfo);
      console.log(`Update decision: ${JSON.stringify(updateDecision, null, 2)}`);
      ...
      } else if (updateDecision.recommendedAction === 'update' || this.config.forceUpdate) {
  ...
  private async analyzeCurrentModel(): Promise<UpdateMetrics> {
    ...
    const accuracyScores = recentValidations.map(v => v.costCorrections?.accuracyScore || 0);
    const currentAccuracy = accuracyScores.reduce((sum, score) => sum + score, 0) / accuracyScores.length;
    
    // Count new samples since last update
    const lastUpdate = await this.getLastUpdateDate();
    const newSamples = recentValidations.filter(v => 
      new Date(v.metadata.routeDate) > lastUpdate
    ).length;
   ...
   */
  private async collectNewTrainingData(): Promise<{ count: number; quality: number; data: any[] }> {
    const client = getMongoClient();
    if (!client) {
      throw new Error("MongoDB not configured");
    }
    ...
    const quality = newData.length > 0 
      ? newData.reduce((sum, d) => sum + (d.costCorrections?.accuracyScore || 0), 0) / newData.length
      : 0;
   ...
   */
  private shouldUpdateModel(currentMetrics: UpdateMetrics, newDataInfo: { count: number; quality: number }): UpdateMetrics {
    ...
    if (currentMetrics.currentAccuracy < this.config.accuracyThreshold) {
      reasons.push(`Low accuracy: ${(currentMetrics.currentAccuracy * 100).toFixed(1)}%`);
      confidence += 0.4;
    ...
    if (newDataInfo.quality > 0.8) {
      reasons.push(`High data quality: ${(newDataInfo.quality * 100).toFixed(1)}%`);
      confidence += 0.2;
    }
   ...
   */
  private async performModelUpdate(metrics: UpdateMetrics): Promise<UpdateResult> {
    ...
    try {
      ...
      if (this.config.dryRun) {
        console.log("DRY RUN: Would train model with", trainingData.length, "samples");
        return {
          success: true,
      ...
      if (validationResult.overallScore > 70) { // Minimum quality threshold
    ...
    try {
      ...
      return {
        ...
        metrics: {
          ...
          currentAccuracy: 100 - validationResult.accuracyMetrics.meanAbsolutePercentageError.cost,
          recommendedAction: 'validate',
  ...
  private async backupCurrentModel(): Promise<string> {
    ...
    const backupPath = path.join(backupDir, `cost-model-backup-${timestamp}.json`);
  ...
  private async deployNewModel(model: TrainedModel): Promise<void> {
    ...
    await fs.writeFile(activeModelPath, JSON.stringify({
  ...
  private async getLastUpdateDate(): Promise<Date> {
    ...
    return lastUpdate ? new Date(lastUpdate.timestamp) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
  }
  ...
  private async getPreviousModel(): Promise<TrainedModel | null> {
    ...
    try {
      ...
      const modelFiles = backupFiles.filter(f => f.startsWith('cost-model-backup-') && f.endsWith('.json'));
  ...
  private async recordUpdateHistory(result: UpdateResult): Promise<void> {
    const historyPath = path.join(process.cwd(), 'models', 'update-history.json');
...
async function main() {
  ...
  console.log(`Current Accuracy: ${(result.metrics.currentAccuracy * 100).toFixed(1)}%`);
  ...
  if (result.newModel) {
    ...
    console.log(`Model Performance: ${(result.newModel.performance.averageAccuracy * 100).toFixed(1)}% accuracy`);
  }
