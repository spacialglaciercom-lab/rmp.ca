...
const CACHE_DIRECTORIES = [".expo", "dist"];
...
function targetedCleanup(dryRun = true) {
  console.log(
    `${dryRun ? "🔍 ANALYZING" : "🧹 CLEANING"} migration artifacts...\n`,
  );
  ...
  CACHE_DIRECTORIES.forEach((dirName) => {
    ...
    if (fs.existsSync(dirPath) && fs.statSync(dirPath).isDirectory()) {
      try {
        ...
        if (dryRun) {
          console.log(
            `   Would clean: ${dirName}/ (${fileCount} files, ${formatFileSize(dirSize)})`,
          );
        } else {
          fs.rmSync(dirPath, { recursive: true, force: true });
          console.log(
            `   Cleaned: ${dirName}/ (${fileCount} files, ${formatFileSize(dirSize)})`,
          );
        }
  ...
  console.log(
    `📁 ${dryRun ? "Would clean" : "Cleaned"}: ${totalCleaned} items`,
  );
  console.log(
    `💾 ${dryRun ? "Would free" : "Freed"}: ${formatFileSize(spaceFreed)}`,
  );
...
function createCleanupSummary(result, dryRun = true) {
  const summary = {
    ...
    artifactsRemoved: MIGRATION_ARTIFACTS.filter((file) => {
      ...
      return (
        !PRESERVE_FILES.includes(file) && (!dryRun || fs.existsSync(filePath))
      );
    }),
  };
  ...
  const summaryPath = path.join(
    process.cwd(),
    "migration-cleanup-summary.json",
  );
