  ...
  try {
    ...
    for (const icon of iosIconSizes) {
      for (const scale of icon.scale) {
        ...
        const outputPath = path.join(iosDir, `processor-icon-${icon.name}-${scale}x.png`);
        ...
        console.log(`✅ Generated ${actualSize}x${actualSize} iOS icon: ${path.basename(outputPath)}`);
      }
    }
    ...
    for (const icon of androidIconSizes) {
      const outputPath = path.join(androidDir, `processor-icon-${icon.density}.png`);
      ...
      console.log(`✅ Generated ${icon.size}x${icon.size} Android icon: ${path.basename(outputPath)}`);
    }
    
    // Generate main app icon (1024x1024)
    const mainIconPath = path.join(__dirname, '../assets/images/processor-icon-main.png');
    await sharp(svgPath)
      .resize(1024, 1024, {
        fit: 'contain',
        background: { r: 0, g: 0, b: 0, alpha: 0 }
    ...
    console.log(`✅ Generated main 1024x1024 app icon: processor-icon-main.png`);
    
    // Generate iOS App Icon JSON configuration
    const iosConfig = generateiOSAppIconConfig();
    const iosConfigPath = path.join(iosDir, 'AppIcon.appiconset', 'Contents.json');
