/**
 * GeneralChatOverlay — Slide-up chat overlay for general-purpose AI chat.
 *
 * Text-only chat (no mic/voice). Uses OpenRouter + Mistral backends.
 * TTS: assistant replies are spoken via ElevenLabs or expo-speech.
 */
...
export const GeneralChatOverlay: React.FC = () => {
  const colors = useColors();
  const insets = useSafeAreaInsets();
  const { state, sendMessage, toggleOverlay, clearMessages, stopSpeaking } = useGeneralChat();
  ...
  // Stop TTS when overlay closes
  useEffect(() => {
    if (!state.isOverlayOpen) {
      stopSpeaking();
    }
  }, [state.isOverlayOpen, stopSpeaking]);
        ...
        >
          ...
          <View style={[styles.header, { borderBottomColor: colors.border }]}>
            ...
            <View style={styles.headerRight}>
              {state.status === "speaking" && (
                <TouchableOpacity onPress={stopSpeaking} style={styles.speakerButton}>
                  <MaterialCommunityIcons name="stop" size={20} color={colors.foreground} />
                </TouchableOpacity>
              )}
            ...
            ListEmptyComponent={
              <View style={styles.emptyContainer}>
                ...
                <Text style={[styles.emptyText, { color: colors.muted }]}>
                  Ask me anything — I'm a general-purpose AI assistant.{"\n"}
