...
function VRPStopMarker({
  ...
  return (
    <View style={[styles.vrpMarkerOuter, { backgroundColor: bgColor }]}>
...
function urlTemplateForNative(url: string): string {
  return url.replace(/\{s\}/g, "a").replace(/\{r\}/g, "");
}
  ...
  forwardRef<RouteMapRef, RouteMapProps>(function RouteMap(
    ...
    useEffect(() => {
      ...
      const processAsync = async () => {
        const lines: typeof osmOverlays extends infer T
          ? T extends null
            ? never
            : NonNullable<T>["lines"]
          : never = [];
    ...
    const navSegmentPolylines = useMemo(() => {
      if (
        navigationSegmentIndex == null ||
        navigationSegmentIndex < 0 ||
      ...
      return {
        ...
        current:
          idx < routeCoords.length - 1 ? routeCoords.slice(idx, idx + 2) : null,
        upcoming:
          idx + 1 < routeCoords.length - 1 ? routeCoords.slice(idx + 1) : null,
      };
    ...
    useEffect(() => {
      if (orientation !== "compass" || !rotateEnabled || Platform.OS === "web")
        return;
      let cancelled = false;
    ...
    useEffect(() => {
      ...
      mapRef.current?.getCamera().then((camera) => {
        if (camera)
          mapRef.current?.setCamera({ ...camera, heading: userBearing });
      });
    ...
    const tileUrlRaw = showAerial
      ...
      : currentBaseLayer?.url ||
        (mapTypePreference === "dark" ? OSM_TILE_DARK : OSM_TILE_STANDARD);
    ...
    return (
      ...
      >
        ...
        >
            ...
            (routePointsByVehicle && routePointsByVehicle.length > 1 ? (
              sanitizeByVehicle(routePointsByVehicle)
                ...
                .map((vehiclePoints, vIdx) => {
                  const coords = vehiclePoints.map((p) => ({
                    latitude: p.lat,
                    longitude: p.lon,
                  }));
              ...
              <>
                {navSegmentPolylines ? (
                  <>
                    {navSegmentPolylines.upcoming &&
                      navSegmentPolylines.upcoming.length >= 2 && (
                        <Polyline
                          key="nav-upcoming"
                    ...
                    {navSegmentPolylines.current &&
                      navSegmentPolylines.current.length >= 2 && (
                        <Polyline
                          key="nav-current"
                    ...
                    {navSegmentPolylines.completed &&
                      navSegmentPolylines.completed.length >= 2 && (
                        <Polyline
                          key="nav-completed"
                  ...
                  filteredRoutePoints.length >= 2 ? (
                  (() => {
                    const riskMap = new Map();
                    segmentRisks.forEach((s) =>
                      riskMap.set(s.segmentIndex, s.riskScore),
                    );
            ...
            (!segmentRisks || segmentRisks.length === 0) && (
              ...
              <>
                ...
                <Marker
                  coordinate={{
                    latitude:
                      filteredRoutePoints[filteredRoutePoints.length - 1].lat,
                    longitude:
                      filteredRoutePoints[filteredRoutePoints.length - 1].lon,
                  }}
                  pinColor="#ef4444"
                  title="End"
                  zIndex={10}
          ...
          {tapDestination &&
            !isNaN(tapDestination.lat) &&
            !isNaN(tapDestination.lon) && (
              <Marker
                coordinate={{
          ...
          {osmExtractionPolygon &&
            ...
            (() => {
              const validPoly = osmExtractionPolygon.filter(
                (p) =>
                  !isNaN(p.latitude) &&
                  !isNaN(p.longitude) &&
                  isFinite(p.latitude) &&
              ...
              const closed =
                validPoly.length > 0 ? [...validPoly, validPoly[0]] : validPoly;
          ...
          {zonesPreviewPolygons && zonesPreviewPolygons.length > 0
            ...
            : zonesPreviewPolygon &&
              ...
              (() => {
                const validPoly = zonesPreviewPolygon.filter(
                  (p) =>
                    !isNaN(p.latitude) &&
                    !isNaN(p.longitude) &&
                    isFinite(p.latitude) &&
              ...
              .map((p) => (
                <Marker
                  ...
                  description={
                    [
                      p.address,
                      p.capacityLiters != null ? `${p.capacityLiters} L` : null,
                      p.condition,
                    ]
                      .filter(Boolean)
                      .join(" · ") || undefined
