...
export const OfflineMapDownloadSection: React.FC = () => {
  ...
  const refresh = useCallback(async () => {
    const [regions, packList] = await Promise.all([
      getDownloadedRegions(),
      getMapLibrePacks(),
    ]);
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem(SOURCE_STORAGE_KEY).then((v) => {
      if (v === "r2" || v === "s3" || v === "osm_pbf" || v === "maplibre_pack")
        setSource(v);
    });
  }, [refresh]);
    ...
    async (city: OfflineCity) => {
      ...
      if (source === "maplibre_pack") {
        ...
        if (packs.some((p) => p.name === packName)) {
          Alert.alert(
            "Already downloaded",
            `${city.name} map tiles are already downloaded. Delete the pack first to re-download.`,
          );
          return;
        }
      ...
      if (source === "osm_pbf") {
        const already = downloaded.some(
          (r) =>
            r.source === "osm_pbf" &&
            (r.cityId === city.id || r.id === `${city.id}_osm_pbf`),
        );
        if (already) {
          Alert.alert(
            "Already downloaded",
            `OSM data for ${city.name} is already downloaded. Delete it first to re-download.`,
          );
          return;
        }
        ...
        try {
          ...
          Alert.alert(
            "Done",
            `OSM data for ${city.name} downloaded for offline routing.`,
          );
        } catch (e) {
          setDownloadProgress(null);
          Alert.alert(
      ...
      try {
        const downloadFn =
          source === "r2" ? downloadCityFromR2 : downloadCityData;
        const result = await downloadFn(
          city,
          (done, total, phase) => {
            setDownloadProgress({ done, total, phase });
          },
          controller.signal,
        );
        setDownloadProgress(null);
  ...
  const handleDeletePack = useCallback(
    (pack: MapLibrePackInfo) => {
      ...
      const cityName = pack.name.startsWith("pack_")
        ? (OFFLINE_CITIES.find((c) => `pack_${c.id}` === pack.name)?.name ??
          pack.name)
        : pack.name;
      confirmDestructive(
        "Delete map tiles",
  ...
  return (
    <View style={{ paddingVertical: 4 }}>
      <Text style={{ fontSize: 13, color: colors.muted, marginBottom: 12 }}>
        Download region for offline: R2/S3 (Overture routing), Map tiles
        (offline viewing), or OSM PBF (offline routing).
      </Text>
      ...
      {(downloaded.length > 0 || packs.length > 0) && (
        <View style={{ marginBottom: 16 }}>
            ...
            >
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ fontSize: 12, color: colors.muted }}>
                  ...
                  {formatBytes(r.sizeBytes)} · {r.country}
                </Text>
          ...
          {packs.map((pack) => {
            const cityName = pack.name.startsWith("pack_")
              ? (OFFLINE_CITIES.find((c) => `pack_${c.id}` === pack.name)
                  ?.name ?? pack.name)
              : pack.name;
            return (
              <View
