...
/**
 * Tear down Manus Runtime - removes message listener. Call on app unmount (e.g. root layout cleanup).
 */
export function destroyManusRuntime(): void {
  if (!isWeb() || !isInIframe()) return;
  if (!initialized) return;
  initialized = false;
  window.removeEventListener("message", handleMessage);
  log("destroyManusRuntime called");
}
