...
try {
  ...
  MapsMeView = ({ style, ...props }: any) => (
    <div style={{ ...style, backgroundColor: '#f0f0f0', borderRadius: 8, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
  ...
  MapsMeModule = {
    initializeFramework: async () => ({ success: false, message: 'Web not supported' }),
...
const MapsMeReactView: React.FC<MapsMeViewProps> = ({
  ...
  useEffect(() => {
    ...
    if (props.latitude !== undefined && props.longitude !== undefined && viewRef.current) {
