...
class MapsMeModuleWeb {
  static async initializeFramework(): Promise<{
    success: boolean;
    message: string;
  }> {
    console.log(
      "MAPS.ME: Web fallback - framework initialization not available",
    );
    return {
      success: false,
      message: "MAPS.ME is only available on native platforms",
    };
  }
  ...
  static async setMapPosition(
    ...
    console.log(
      `MAPS.ME: Web fallback - setMapPosition(${latitude}, ${longitude}) not available`,
    );
  }
