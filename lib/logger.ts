...
export function createLogger(namespace: string): NamespacedLogger {
  ...
  return {
    ...
    error(msg, errorOrData?, data?) {
      ...
      if (errorOrData instanceof Error) _tryRecordCrashlytics(errorOrData, namespace);
    },
  };
}

// ── Backward-compat Logger class + singleton ──────────────────────────────────
interface LoggerConfig { enabled: boolean; level: LogLevel; includeTimestamp: boolean; includeContext: boolean; }
const DEFAULT_CONFIG: LoggerConfig = { enabled: true, level: _resolveLevel(), includeTimestamp: true, includeContext: true };

export class Logger {
  private config: LoggerConfig;
  constructor(config: Partial<LoggerConfig> = {}) { this.config = { ...DEFAULT_CONFIG, ...config }; }
  private shouldLog(l: LogLevel) { return this.config.enabled && _shouldLog(this.config.level, l); }
  private fmt(l: LogLevel, msg: string, ctx?: string) {
    ...
    parts.push(msg); return parts.join(" ");
  }
  debug(msg: string, ctx?: string) { if (this.shouldLog("debug")) console.debug(this.fmt("debug", msg, ctx)); }
  info(msg: string, ctx?: string)  { if (this.shouldLog("info"))  console.info(this.fmt("info",  msg, ctx)); }
  warn(msg: string, ctx?: string)  { if (this.shouldLog("warn"))  console.warn(this.fmt("warn",  msg, ctx)); }
  error(msg: string, err?: unknown, ctx?: string) {
    if (!this.shouldLog("error")) return;
    err !== undefined ? console.error(this.fmt("error", msg, ctx), err) : console.error(this.fmt("error", msg, ctx));
    if (err instanceof Error) _tryRecordCrashlytics(err, ctx ?? "app");
  }
  setConfig(c: Partial<LoggerConfig>) { this.config = { ...this.config, ...c }; }
