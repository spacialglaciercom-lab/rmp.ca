...
export class CostCorrectionModel {
  ...
  private costCache = new Map<string, { prediction: CostPrediction; timestamp: number }>();
  private historicalData: Array<{ factors: CostFactors; actual: ActualCostData }> = [];
  ...
  async predictCosts(
    ...
    if (Platform.OS !== "ios") {
      return this.getFallbackPrediction(routeStats, weatherData, operationalContext);
    }

    const costFactors = this.buildCostFactors(routeStats, weatherData, operationalContext);
  ...
  async correctCosts(
    ...
    const prediction = await this.predictCosts(routeStats, weatherData, operationalContext);
  ...
  private buildCostFactors(
    ...
    const totalTurns = Math.max(1, turns.leftTurns + turns.rightTurns + turns.uTurns + turns.straightAhead);
    ...
    return {
      ...
      // Operational factors
      vehicleType: operationalContext?.vehicleType ?? "municipal waste collection truck",
   ...
   */
  private async callAICostModel(costFactors: CostFactors): Promise<CostPrediction | null> {
  ...
  private getFallbackPrediction(
    ...
    // Time adjustments
    const timeMultiplier = this.getTimeOfDayMultiplier(operationalContext?.timeOfDay);
    
    // Driver experience adjustments
    const experienceMultiplier = this.getExperienceMultiplier(operationalContext?.driverExperience);
    
    const adjustedTime = baseTime * weatherMultiplier * timeMultiplier * experienceMultiplier;
   ...
   */
  private generateCorrectionReasoning(correctionFactor: number, prediction: CostPrediction): string {
  ...
  private getFallbackSuggestions(weatherData?: CurrentWeather): string[] {
    ...
    if (weatherData) {
      if (weatherData.rain1h > 5) suggestions.push("Consider delaying start to avoid peak precipitation");
      if (weatherData.windSpeed > 10) suggestions.push("Allow extra time for windy conditions");
      if (weatherData.visibility < 1000) suggestions.push("Reduce speed in low visibility areas");
    }
   ...
   */
  private updateHistoricalAccuracy(predicted: CostPrediction, actual: ActualCostData): void {
    const timeAccuracy = 1 - Math.abs(predicted.predictedTime - actual.actualTime) / predicted.predictedTime;
    const costAccuracy = 1 - Math.abs(predicted.predictedTotalCost - actual.totalCost) / predicted.predictedTotalCost;
