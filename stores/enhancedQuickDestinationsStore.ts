...
interface EnhancedQuickDestinationsStore {
  ...
  updateCustomDestination: (
    id: string,
    updates: Partial<QuickDestination>,
  ) => void;
...
export const useEnhancedQuickDestinationsStore =
  create<EnhancedQuickDestinationsStore>()(
    persist(
      (set, get) => ({
        home: null,
        work: null,
        ...
        deleteCustomDestination: (id) => {
          set((state) => ({
            customDestinations: state.customDestinations.filter(
              (dest) => dest.id !== id,
            ),
          }));
        },
        ...
        addRecentDestination: (dest) => {
          set((state) => {
            ...
            let filteredRecent = state.recentDestinations.filter(
              (recent) =>
                !(
                  recent.coords.lat === dest.coords.lat &&
                  recent.coords.lon === dest.coords.lon
                ),
            );
            ...
            filteredRecent = filteredRecent.slice(
              0,
              state.maxRecentDestinations,
            );
        ...
        getDestinationById: (id) => {
          ...
          const customDest = state.customDestinations.find(
            (dest) => dest.id === id,
          );
          if (customDest) return customDest;

          const recentDest = state.recentDestinations.find(
            (dest) => dest.id === id,
          );
          return recentDest;
        },
