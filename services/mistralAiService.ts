...
import type { RouteStats, AIRouteAnalysis } from '@/services/weatherAiRouteAnalysis';
 ...
 */
async function callMistralApi(prompt: string, systemMessage: string): Promise<string | null> {
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        !["none", "low", "moderate", "high"].includes(parsedResponse.weatherImpactSeverity) ||
      ...
      if (parsedResponse.confidenceScore < 0 || parsedResponse.confidenceScore > 1) {
 ...
 */
export function getMistralCacheStats(): { size: number; entries: Array<{ key: string; age: number }> } {
  const now = Date.now();
