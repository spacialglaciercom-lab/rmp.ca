...
const FALLBACK_MODELS = ["openai/gpt-4o-mini", "openai/gpt-4o", "openai/gpt-4.1-mini"] as const;
...
export function registerAiProxyRoutes(app: Express) {
  ...
  app.post("/api/ai/chat", async (req: Request, res: Response) => {
    ...
    try {
      const { messages, model, systemPrompt, max_tokens, temperature } = req.body as {
      ...
      const firstModel = isAnthropicModel(requestedModel) ? DEFAULT_MODEL : requestedModel;
      ...
      try {
        ...
        const cause = streamErr instanceof Error && streamErr.cause ? String(streamErr.cause) : "";
        log.error("AI Gateway stream error", { message: msg, cause: cause || undefined });
        const status = typeof (streamErr as { status?: number })?.status === "number" ? (streamErr as { status: number }).status : 500;
      ...
      if (reply) {
        ...
        res.status(502).json({ error: "Empty or invalid response from AI Gateway" });
      }
    } catch (err) {
