...
export function registerMapsProxyRoutes(app: Express) {
  ...
  app.post("/api/maps/directions", async (req: Request, res: Response) => {
    ...
    try {
      const { origin, destination, waypoints, mode, avoid, departureTime } = req.body as {
        origin: string;
        destination: string;
        waypoints?: string;
        mode?: string;
      ...
      const avoidStr = Array.isArray(avoid) ? avoid.filter(Boolean).join("|") : avoid;
      if (avoidStr) url += `&avoid=${encodeURIComponent(avoidStr)}`;
      if (departureTime === "now" || (typeof departureTime === "number" && departureTime > 0)) {
      ...
      log.error("directions error", err instanceof Error ? err : new Error(String(err)));
  ...
  app.post("/api/maps/snap-to-roads", async (req: Request, res: Response) => {
    ...
    try {
      ...
      log.error("snap-to-roads error", err instanceof Error ? err : new Error(String(err)));
  ...
  app.post("/api/maps/places/autocomplete", async (req: Request, res: Response) => {
    ...
    try {
      ...
      log.error("places autocomplete error", err instanceof Error ? err : new Error(String(err)));
  ...
  app.post("/api/maps/places/nearby", async (req: Request, res: Response) => {
    ...
    try {
      const { lat, lon, radiusMeters } = req.body as { lat?: number; lon?: number; radiusMeters?: number };
      ...
      log.error("places nearby error", err instanceof Error ? err : new Error(String(err)));
   ...
   */
  app.get("/api/maps/places/details/:placeId", async (req: Request, res: Response) => {
    const apiKey = ENV.googleMapsApiKey;
    if (!apiKey) {
      res.status(503).json({ error: "Google Maps API key not configured on server" });
      return;
    ...
    try {
      ...
      const fields = (req.query.fields as string) || "id,displayName,formattedAddress,location";
      ...
      log.error("places details error", err instanceof Error ? err : new Error(String(err)));
