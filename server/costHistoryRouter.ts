...
const CostHistoryEntrySchema = z.object({
  ...
  costFactors: z.object({
    ...
    operationalContext: z.object({
      ...
      driverExperience: z.enum(["novice", "intermediate", "experienced"]).optional(),
      timeOfDay: z.enum(["early_morning", "morning", "afternoon", "evening"]).optional(),
...
export const costHistoryRouter = router({
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        const costCorrections = {
          timeCorrectionFactor: input.predictedCosts.predictedTime !== 0 ? input.actualCosts.actualTime / input.predictedCosts.predictedTime : 1,
          costCorrectionFactor: input.predictedCosts.predictedTotalCost !== 0 ? input.actualCosts.totalCost / input.predictedCosts.predictedTotalCost : 1,
          fuelCorrectionFactor: input.predictedCosts.predictedFuelConsumption !== 0 ? input.actualCosts.actualFuelConsumption / input.predictedCosts.predictedFuelConsumption : 1,
          accuracyScore: input.predictedCosts.predictedTotalCost !== 0 ? Math.max(0, 1 - Math.abs(input.actualCosts.totalCost - input.predictedCosts.predictedTotalCost) / input.predictedCosts.predictedTotalCost) : 0,
          primaryErrorSources: identifyPrimaryErrorSources(input.predictedCosts, input.actualCosts, input.costFactors),
        };
  ...
  getTrainingData: publicProcedure
    .input(z.object({
      ...
      weatherConditions: z.array(z.enum(["clear", "rain", "snow", "fog"])).optional(),
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        if (input.startDate || input.endDate) {
          ...
          if (input.startDate) filter["metadata.routeDate"].$gte = input.startDate;
          if (input.endDate) filter["metadata.routeDate"].$lte = input.endDate;
        }
        
        if (input.minAccuracyScore !== undefined) {
          filter["costCorrections.accuracyScore"] = { $gte: input.minAccuracyScore };
        }
        
        if (input.vehicleTypes && input.vehicleTypes.length > 0) {
          filter["costFactors.operationalContext.vehicleType"] = { $in: input.vehicleTypes };
        }
        
        if (input.weatherConditions && input.weatherConditions.length > 0) {
          filter["costFactors.weatherConditions.condition"] = { $in: input.weatherConditions };
        }
        
        if (input.routeComplexity) {
          filter["costFactors.routeCharacteristics.turnComplexity"] = input.routeComplexity === "low" ? { $lte: 0.33 } : input.routeComplexity === "high" ? { $gte: 0.66 } : { $gt: 0.33, $lt: 0.66 };
        }
  ...
  getAccuracyStats: publicProcedure
    .input(z.object({
      startDate: z.date().optional(),
      endDate: z.date().optional(),
      groupBy: z.enum(["day", "week", "month", "vehicle_type", "weather_condition"]).default("week"),
    }))
    .query(async ({ input }) => {
      const client = getMongoClient();
      if (!client) {
      ...
      try {
        ...
        switch (input.groupBy) {
          case "day":
            groupByField = { $dateToString: { format: "%Y-%m-%d", date: "$metadata.routeDate" } };
            break;
          case "week":
            groupByField = { $dateToString: { format: "%Y-%W", date: "$metadata.routeDate" } };
            break;
          case "month":
            groupByField = { $dateToString: { format: "%Y-%m", date: "$metadata.routeDate" } };
            break;
          case "vehicle_type":
        ...
        const stats = await collection.aggregate([
          { $match: filter },
          {
            $group: {
              _id: groupByField,
              count: { $sum: 1 },
              avgTimeAccuracy: { $avg: { $subtract: [1, { $abs: { $subtract: ["$costCorrections.timeCorrectionFactor", 1] } }] } },
              avgCostAccuracy: { $avg: { $subtract: [1, { $abs: { $subtract: ["$costCorrections.costCorrectionFactor", 1] } }] } },
              avgFuelAccuracy: { $avg: { $subtract: [1, { $abs: { $subtract: ["$costCorrections.fuelCorrectionFactor", 1] } }] } },
              avgAccuracyScore: { $avg: "$costCorrections.accuracyScore" },
              totalPredictedCost: { $sum: "$predictedCosts.predictedTotalCost" },
              totalActualCost: { $sum: "$actualCosts.totalCost" },
              avgPredictedCost: { $avg: "$predictedCosts.predictedTotalCost" },
        ...
        return {
          accuracyStats: stats,
          overallAccuracy: stats.length > 0 ? stats.reduce((acc, curr) => acc + curr.avgAccuracyScore, 0) / stats.length : 0,
          totalBias: stats.reduce((acc, curr) => acc + (curr.totalActualCost - curr.totalPredictedCost), 0),
        };
      } catch (error) {
  ...
  getCostFactorAnalysis: publicProcedure
    .input(z.object({
      factor: z.enum(["weather", "driver_experience", "route_complexity", "time_of_day", "season"]),
      metric: z.enum(["time_correction", "cost_correction", "fuel_correction", "accuracy_score"]).default("accuracy_score"),
      startDate: z.date().optional(),
      endDate: z.date().optional(),
    }))
    .query(async ({ input }) => {
      ...
      try {
        ...
        return {
          ...
          insights: generateFactorInsights(analysis, input.factor, input.metric),
        };
      } catch (error) {
  ...
  exportCostTrainingData: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        filter["costCorrections.accuracyScore"] = { $gte: input.minSampleQuality };
        ...
        const trainingData = entries.map(doc => ({
          ...
          route_features: {
            ...
            turn_complexity: (doc.routeStatistics.turns.leftTurns * 2 + doc.routeStatistics.turns.uTurns * 3) / 
              Math.max(1, doc.routeStatistics.turns.leftTurns + doc.routeStatistics.turns.rightTurns + doc.routeStatistics.turns.uTurns + doc.routeStatistics.straightAhead),
            segment_density: doc.routeStatistics.totalDistance > 0 ? doc.routeStatistics.segmentsRouted / doc.routeStatistics.totalDistance : 0,
            efficiency_ratio: doc.routeStatistics.segmentsRouted / Math.max(1, doc.routeStatistics.segmentsRouted + doc.routeStatistics.segmentsExcluded),
          },
          weather_features: input.includeWeather && doc.costFactors.weatherConditions ? {
            temperature: doc.costFactors.weatherConditions.temperature,
            precipitation: doc.costFactors.weatherConditions.precipitation,
            ...
            condition_encoded: encodeWeatherCondition(doc.costFactors.weatherConditions.condition),
          } : null,
          operational_features: {
            vehicle_type_encoded: encodeVehicleType(doc.costFactors.operationalContext.vehicleType),
            driver_experience_encoded: encodeDriverExperience(doc.costFactors.operationalContext.driverExperience),
            time_of_day_encoded: encodeTimeOfDay(doc.costFactors.operationalContext.timeOfDay),
            day_of_week: doc.costFactors.operationalContext.dayOfWeek,
            season_encoded: encodeSeason(doc.costFactors.operationalContext.season),
            route_complexity_encoded: encodeRouteComplexity(doc.costFactors.routeCharacteristics.urbanDensity),
          },
          ...
          targets: {
            ...
            actual_cost_per_km: doc.actualCosts.totalCost / doc.routeStatistics.totalDistance,
            actual_time_per_km: doc.actualCosts.actualTime / doc.routeStatistics.totalDistance,
            actual_fuel_per_km: doc.actualCosts.actualFuelConsumption / doc.routeStatistics.totalDistance,
          },
          metadata: {
        ...
        return {
          ...
          format: input.format,
          avgAccuracy: trainingData.length > 0 ? trainingData.reduce((acc, curr) => acc + curr.metadata.accuracy_score, 0) / trainingData.length : 0,
        };
      } catch (error) {
...
// Helper functions
function identifyPrimaryErrorSources(predicted: any, actual: any, factors: any): string[] {
  const errors: string[] = [];
  
  const timeError = predicted.predictedTime !== 0 ? Math.abs(actual.actualTime - predicted.predictedTime) / predicted.predictedTime : 0;
  const costError = predicted.predictedTotalCost !== 0 ? Math.abs(actual.totalCost - predicted.predictedTotalCost) / predicted.predictedTotalCost : 0;
  const fuelError = predicted.predictedFuelConsumption !== 0 ? Math.abs(actual.actualFuelConsumption - predicted.predictedFuelConsumption) / predicted.predictedFuelConsumption : 0;
  ...
  if (factors.weatherConditions?.severity > 0.7) errors.push("Severe weather impact underestimated");
  if (factors.operationalContext.driverExperience === "novice") errors.push("Novice driver performance variance");
  
  return errors.length > 0 ? errors : ["Minor prediction variance"];
}

function generateFactorInsights(analysis: any[], factor: string, metric: string): string[] {
  ...
  insights.push(`${factor}: Best performance in "${best._id || 'unknown'}" (${best.avgMetric.toFixed(3)}), worst in "${worst._id || 'unknown'}" (${worst.avgMetric.toFixed(3)})`);
  
  if (best.correlation && worst.correlation) {
    insights.push(`Correlation strength: ${best._id || 'unknown'} (${best.correlation.toFixed(3)}) vs ${worst._id || 'unknown'} (${worst.correlation.toFixed(3)})`);
  }
