...
export const costHistoryRouter = router({
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        const result = await collection.insertOne({
          ...input,
          costCorrections,
          // Scope to org (or solo user as fallback) for multi-tenant isolation
  ...
  getTrainingData: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      try {
        ...
        // Scope query to this org (or solo user) — multi-tenant isolation
        if (ctx.org?.id != null) {
          filter.orgId = ctx.org.id;
        } else {
  ...
  getCostFactorAnalysis: protectedProcedure
    .input(
      z.object({
        factor: z.enum([
          "weather",
  ...
  exportCostTrainingData: protectedProcedure
    .input(
      z.object({
