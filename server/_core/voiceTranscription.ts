...
/**
 * Transcribe audio from base64-encoded data (for native iOS/Android uploads).
 * Skips the URL download step and sends audio directly to Whisper.
 */
export async function transcribeAudioFromBase64(
  ...
  try {
    ...
    formData.append("model", "whisper-1");
    formData.append("response_format", "verbose_json");
    formData.append("prompt", prompt || (language ? `Transcribe the user's voice to text, the user's working language is ${getLanguageName(language)}` : "Transcribe the user's voice to text"));
