...
# bastille cp uses tar under the hood; use it for directory trees.
# For single files bastille cmd cp works fine.
jail_cp_dir() {
    local jail="$1" src="$2" dst="$3"
    tar -C "${src}" -cf - . | bastille cmd "${jail}" sh -c "mkdir -p ${dst} && tar -C ${dst} -xf -"
}

jail_path() {
    bastille list | awk -v j="$1" '$1==j {print $NF}'
}

# ── 1. Extract service ────────────────────────────────────────────────────────

info "Deploying rmpca-extract"
bastille cmd rmpca-extract mkdir -p /app/extract
jail_cp_dir rmpca-extract "${REPO_ROOT}/extract" /app/extract
bastille cmd rmpca-extract sh -c "cd /app/extract && npm install --omit=dev"
bastille cmd rmpca-extract sh -c "chown -R www:www /app/extract"
bastille cmd rmpca-extract service extract restart || bastille cmd rmpca-extract service extract start

# ── 2. Node.js API server (backend) ──────────────────────────────────────────

info "Deploying rmpca-backend"
bastille cmd rmpca-backend mkdir -p /app
jail_cp_dir rmpca-backend "${REPO_ROOT}" /app

bastille cmd rmpca-backend sh -c "
    cd /app
...
bastille cmd rmpca-backend sh -c "cat > /usr/local/etc/rmpca/backend.env <<'ENV'
NODE_ENV=production
PORT=3000
...
if [ -f "${REPO_ROOT}/.env.server" ]; then
    ...
    bastille cmd rmpca-backend sh -c "cat >> /usr/local/etc/rmpca/backend.env" \
        < "${REPO_ROOT}/.env.server"
fi

bastille cmd rmpca-backend service backend restart || bastille cmd rmpca-backend service backend start
...
if [ "${OPTIMIZER}" -eq 1 ]; then
    ...
    bastille template rmpca-redis "$(dirname "$0")/redis"
    bastille cmd rmpca-redis service redis restart || bastille cmd rmpca-redis service redis start

    for jail in rmpca-optimizer rmpca-celery; do
        info "Deploying ${jail}"
        bastille cmd "${jail}" mkdir -p /app/backend
    ...
    bastille cmd rmpca-optimizer sh -c "cat > /usr/local/etc/rmpca/optimizer.env <<'ENV'
PORT=8000
REDIS_URL=redis://10.10.0.4:6379/0
ENV"

    # celery.env
    bastille cmd rmpca-celery sh -c "cat > /usr/local/etc/rmpca/celery.env <<'ENV'
REDIS_URL=redis://10.10.0.4:6379/0
$([ -n "${DEM_PATH:-}" ] && echo "DEM_PATH=${DEM_PATH}" || true)
ENV"

    bastille cmd rmpca-optimizer service optimizer restart || bastille cmd rmpca-optimizer service optimizer start
    bastille cmd rmpca-celery     service celery    restart || bastille cmd rmpca-celery     service celery    start
    ...
    bastille cmd rmpca-nginx-opt service nginx restart || bastille cmd rmpca-nginx-opt service nginx start
...
if [ "${MOONSHINE}" -eq 1 ]; then
    ...
    bastille cmd rmpca-moonshine mkdir -p /app/moonshine
    jail_cp_dir rmpca-moonshine "${REPO_ROOT}/server/moonshine-sidecar" /app/moonshine
    bastille cmd rmpca-moonshine sh -c "
    ...
    bastille cmd rmpca-moonshine service moonshine restart || bastille cmd rmpca-moonshine service moonshine start
...
for jail in rmpca-extract rmpca-backend; do
    bastille cmd "${jail}" service "$(echo "${jail}" | sed 's/rmpca-//')" status 2>/dev/null || true
done
[ "${OPTIMIZER}" -eq 1 ] && for jail in rmpca-redis rmpca-optimizer rmpca-celery rmpca-nginx-opt; do
    ...
    bastille cmd "${jail}" service "${svc}" status 2>/dev/null || true
done
[ "${MOONSHINE}" -eq 1 ] && bastille cmd rmpca-moonshine service moonshine status 2>/dev/null || true
